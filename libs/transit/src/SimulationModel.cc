...
SimulationModel::SimulationModel(IController& controller)
  ...
  entityFactory.AddFactory(new RadioTowerFactory());
  entityFactory.AddFactory(new SignalBorderFactory());
}
...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  // if the entity being created is a radio tower, link it to the controller
  // this will allow the radio tower to push text to the notification bar
  if ((std::string) entity["type"] == "radio tower") {
    towers.push_back(dynamic_cast<RadioTower*>(myNewEntity));
    dynamic_cast<RadioTower*>(myNewEntity)->linkController(&this->controller);
  }
